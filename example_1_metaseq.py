...
features, arrays ,tsses , tsses_1kb = Pyntegrate.chipSeqSignalAnalysis.generate_arrays_features_from_tsses_from_db(os.path.join(path, 'metaseq-example/data/Homo_sapiens.GRCh37.66_chr17.gtf.db'), os.path.join(path, 'metaseq-example/data/bamData/wgEncodeHaibTfbsK562Atf3V0416101AlnRep1_chr17.bam'),'bam',os.path.join(path, 'metaseq-example/data/bamData/wgEncodeHaibTfbsK562RxlchV0416101AlnRep1_chr17.bam'),'bam',genome = "hg19")
...
control = Pyntegrate.ResultsTable(
    os.path.join("metaseq-example/data/", 'GSM847565_SL2585.table'),
    import_kwargs=dict(index_col=0))

knockdown = Pyntegrate.ResultsTable(
    os.path.join("metaseq-example/data/", 'GSM847566_SL2592.table'),
    import_kwargs=dict(index_col=0))


print("\nControl: ", control.head(),"Control len: ", len(control.data) )
print("\nknockdown: ", knockdown.head(),"knockdown len: ", len(knockdown.data) )

control = control.reindex_to(tsses, attribute='transcript_id')
knockdown = knockdown.reindex_to(tsses, attribute='transcript_id')

print("\nLen de cointrol after reindex: ", len(control))
