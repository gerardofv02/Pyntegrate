...
ap.add_argument(
    ...
    'bigbed].')
ap.add_argument(
    '--prefix', default=os.path.join(Pyntegrate.data_dir(), 'x'),
...
for req in requested:
    if req != 'bed':
        signals.append(Pyntegrate.genomic_signal(args.prefix + '.' + req, req))
    else:
        signals.append(Pyntegrate.genomic_signal(args.prefix + '.bed.gz', 'bed'))
