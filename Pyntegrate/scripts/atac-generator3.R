...
# file_name <- paste0("test_3_DNase-seq", ".bw")
# write.table(custom_dnaseseq, file = file_name, sep = "\t")
custom_rnaseq<-sampleData$SimRNAseq$data
file_name <- paste0("test_3_RNA-seq_3", ".csv")
write.csv(custom_rnaseq, file = file_name, sep = "\t")

# custom_dnaseseq<-head(sampleData$SimATACseq$data,100)
# file_name <- paste("test_3_ATAC-seq", ".bed")
# write.table(custom_dnaseseq, file = file_name, sep = "\t")

# custom_dnaseseq<-head(sampleData$SimChIPseq$data,100)
# file_name <- paste("test_3_CHIP-seq", ".bed")
# write.table(custom_dnaseseq, file = file_name, sep = "\t")
...
# file_name <- "test_3_ChIP-seq_3.bw"
